# FullStack

个人全栈开发知识库。

这个站点用于沉淀 Web、Frontend、Backend、Client、DevOps 和项目复盘。旧内容目前保留在各主题的 `legacy/` 下，后续会逐篇重写、删除或合并。

## Web

- [web_foundation](./web_foundation/) - HTTP, DNS, domain, hosting, and how the Web works.
- [web_security](./web_security/) - XSS, CSRF, CORS, HTTPS, cookies, sessions, and application security.

## Frontend

- [frontend_html](./frontend_html/)
- [frontend_css](./frontend_css/)
- [frontend_javascript](./frontend_javascript/)
- [frontend_typescript](./frontend_typescript/)
- [frontend_browser](./frontend_browser/)
- [frontend_ui](./frontend_ui/)
- [frontend_vue](./frontend_vue/)
- [frontend_react](./frontend_react/)
- [frontend_rendering](./frontend_rendering/)
- [frontend_build](./frontend_build/)
- [frontend_testing](./frontend_testing/)
- [frontend_performance](./frontend_performance/)
- [frontend_accessibility](./frontend_accessibility/)

## Backend

- [backend_runtime](./backend_runtime/)
- [backend_api](./backend_api/)
- [backend_web_server](./backend_web_server/)
- [backend_database](./backend_database/)
- [backend_cache](./backend_cache/)
- [backend_search](./backend_search/)
- [backend_realtime](./backend_realtime/)
- [backend_message_queue](./backend_message_queue/)
- [backend_auth](./backend_auth/)
- [backend_testing](./backend_testing/)
- [backend_architecture](./backend_architecture/)
- [backend_observability](./backend_observability/)
- [backend_scalability](./backend_scalability/)

## Client

- [client_mobile](./client_mobile/)
- [client_desktop](./client_desktop/)

## DevOps

- [devops_git](./devops_git/)
- [devops_linux](./devops_linux/)
- [devops_docker](./devops_docker/)
- [devops_deploy](./devops_deploy/)

## Practice

- [projects](./projects/)
- [archive](./archive/)
