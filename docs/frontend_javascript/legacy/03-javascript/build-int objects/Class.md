...
1.创建一个新的对象2.`constructor`使用给定的参数运行,并将参数挂载到`this`上
...
- 二者均不会像函数声明一样被提升
- 两者的name属性值不同，匿名类表达式的name属性值是一个空字符串，而类声明的name属性值为类名
...
但它们之间存在巨大的差异: 1.通过 `class` 创建的函数具有特殊的内部属性标记 `[[IsClassConstructor]]: true` 2.类的方法不可枚举.类定义将 `"prototype"` 中的所有方法的 `enumerable` 标志设置为 `false` 3.类中总是使用'use strict'.
...
1.初始化父类属性,赋予默认值2.执行父类构造函数3.初始化子类属性,赋予默认值4.执行子类构造函数
...
1.如果现有的类不具备多态行为,就用工厂函数创建之,令工厂函数返回恰当的对象实例2.在**调用方**代码中使用**工厂函数**获得对象实例。3.将带有条件逻辑的函数移到超类中。如果条件逻辑还未提炼至独立的函数，首先对其使用提炼函数。4.任选一个子类，在其中建立一个函数，使之覆写超类中容纳条件表达式的那个函数。将与该子类相关的条件表达式分支复制到新函数中，并对它进行适当调整。
5，重复上述过程，处理其他条件分支。
