...
另做一道不用写代码的推演题：把本章开头「10 个请求全部收到 401」的场景套在 8.6 的
变体 (a) 上，推演三个问题——`refreshCredential` 会被调几次？登录页会跳几次？冷却
窗口内随后到达的 401 拿到什么？（答案都藏在那五组状态里：一次；一次；复用缓存的
失败，不再打续期接口。）
